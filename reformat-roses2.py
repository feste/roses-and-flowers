...
targets = {"target0":["roses", "daffodils", "flowers"]
  , "target1": ["biologists", "paleontologists", "scientists"]
  , "target2": ["beef", "veal", "meat"]
  , "target3": ["oaks", "spruces", "trees"]
  , "target4": ["surgeons", "anesthesiologists", "doctors"]
  , "target5": ["horse", "waterfowl", "animal"]} 

f = open("roses-5-20-mturk.results")
line_number = 0
data = [["subject", "rating", "order", "frequency", "hasOther", "filler", "item", "sentence", "rt", "qNumber", "age", "language"]]
...
for line in f:
  ...
  if line_number == 0:
  ...
  elif line_number > 140:
    #get data
    subject_data = {}
    trial_data = {"rating":[], "rt":[], "qNumber":[], "sentence":[], "item":[], "order":[], "frequency":[], "hasOther":[], "filler":[]}
          ...
          for trial in element:
            ...
            if type(trialType)==type(u'trialType'):
              if trialType[:-1] == "filler":
                ...
                trial_data["item"].append("NA")
                trial_data["sentence"].append("NA")
              elif trialType[:-1] == "target":
                freq, infreq, superset = targets[trialType]
                rating = str(trial["response"][-1])
                rt = str(trial["rt"][-1])
                qNumber = str(trial["qNumber"])
                ...
                pattern_string = ".*(" + \
                  ...
                  infreq + "|" + infreq.capitalize() + "|" + \
                  superset + "|" + superset.capitalize() + \
                    ")( and other | and )(" + \
                    freq + "|" + infreq + "|" + superset + ").*"
                pattern = re.compile(pattern_string)
                m = re.match(pattern, sentence)
                if m.group(1) == freq or m.group(1) == freq.capitalize():
                  order = "subSuper"
                  frequency = "frequent"
                elif m.group(1) == infreq or m.group(1) == infreq.capitalize():
                  order = "subSuper"
                  frequency = "infrequent"
                elif m.group(1) == superset or m.group(1) == superset.capitalize():
                  order = "superSub"
                  if m.group(3) == freq:
                    frequency = "frequent"
                  elif m.group(3) == infreq:
                    frequency = "infrequent"
                  else:
                    print "Error 12"
                else:
                  print "Error 13"
                if m.group(2) == " and other ":
                  hasOther = "True"
                elif m.group(2) == " and ":
                  hasOther = "False"
                else:
                ...
                trial_data["hasOther"].append(hasOther)
                trial_data["qNumber"].append(qNumber)
                trial_data["item"].append(trial["superset"])
                trial_data["sentence"].append(sentence)
            else:
    ...
    for k in range(len(trial_data["rating"])):
      ...
      sentence = trial_data["sentence"][k]
      frequency = trial_data["frequency"][k]
      hasOther = trial_data["hasOther"][k]
      item = trial_data["item"][k]
      subject = subject_data["subject"]
      comments = subject_data["comments"]
      age = subject_data["age"]
      language = subject_data["language"]
      data.append([subject, rating, order, frequency, hasOther, filler, item, sentence, rt, qNumber, age, language])
